...
const RepoCard = ({
  ...
  const linkClasses = "hover:text-blue-400 transition-colors";
  ...
  return (
    <div className={baseClasses}>
      <div className="flex items-center justify-between mb-3">
        {isEnhanced ? (
          <>
            ...
            {showOwner && repo.owner && (
              <p className="text-gray-500">by {repo.owner.login}</p>
            )}
          </>
        ) : (
          <h4 className="text-lg font-semibold text-white flex-1 truncate">
            <a
              href={repo.html_url}
              target="_blank"
              rel="noopener noreferrer"
        ...
        {repo.private && (
          <span
            className={`ml-2 py-1 text-xs bg-yellow-600 text-white rounded ${isEnhanced ? "shrink-0" : ""}`}
          >
            Privado
          </span>
        )}
